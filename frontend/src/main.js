...
import {SelectFiles,SelectGeojsonFileForGeoreference, ProcessFiles, ConnectToDB, DisconnectDB,SaveDbConfig,LoadDbConfig,GetMasterMaps,SelectGeojsonFile,CreateMasterMaps,DeleteMasterMap} from '../wailsjs/go/main/App';

document.querySelector('#app').innerHTML = `
  <main class="container" role="main">
      
      <h1><span class="material-icons text-2xl mr-2 text-blue-500">map</span> Geomatis</h1>
      ...
      <section class="input-box " role="tabpanel" aria-labelledby="tab-database" id="database" tabindex="0">
        ...
        <div style="display:flex; gap: 12px; margin-top:12px;">
          <button type="button" class="btn" id="connectDbBtn" onclick="connectDatabase()">
            <span class="material-icons" aria-hidden="true">link</span>Connect to Database
          </button>
          <button type="button" class="btn warning-btn" id="disconnectDbBtn" style="display: none;" onclick="disconnectDatabase()">
            <span class="material-icons" aria-hidden="true">link_off</span>Disconnect Database
          </button>
          <button type="button" class="save-btn btn" id="saveDbBtn" onclick="saveDatabaseConfig()">
            <span class="material-icons" aria-hidden="true">save</span>Save Settings
          </button>
        </div>
      </section>
      ...
      <section class="input-box" role="tabpanel" aria-labelledby="tab-master" id="master" tabindex="0" hidden>
        ...
        <div class="button-bar" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
          <button class="warning-btn  btn" type="button" id="uploadMasterMapBtn" onclick="uploadMasterMap()">
            <span class="material-icons" aria-hidden="true">upload_file</span>Upload Master Maps
          </button>
          <button class="refresh-btn btn" type="button" id="refreshMasterMapBtn" onclick="refreshMasterMapsTable()">
            <span class="material-icons" aria-hidden="true">refresh</span>Refresh
          </button>
        </div>
      ...
      <section class="input-box active" role="tabpanel" aria-labelledby="tab-georeferensi" id="georeferensi" tabindex="0" hidden>
        ...
        <div class="map-option">
          ...
          <div><label for="ws-type">WS Map</label></div>
          <input type="radio" id="wb-type" name="masterMapType" value="wb">
          <div><label for="wb-type">WB Map</label></div>
        </div>

        <label style="display: block;margin-top:5px">
          Choose Master Map Source
        </label>
        <div class="source-option">
          <input type="radio" id="file-source" name="masterMapSource" value="file" checked="checked">
          <div><label for="file-source">From File</label></div>
          <input type="radio" id="database-source" name="masterMapSource" value="database">
          <div><label for="database-source">From Database</label></div>
        </div>
          
        <div class="file-source-input">
          <label style="display: block; margin-bottom: 15px; margin-top:5px">
            Select Master Map
          </label>
          <button class="btn upload-btn w-full"id="selectGeojsonButton" onclick="selectGeojson()"><span class="material-icons">upload</span> Select a geojson file</button>
          <div class="select-files" id="geojsonFile"style="padding:0">
            No geojson selected
          </div>
        </div>
        ...
        <div>
          ...
          <button class="btn upload-btn w-full"id="selectFileButton"onclick="selectFiles()"><span class="material-icons">upload</span> Select raster files (jpg/png)</button>
          <div class="select-files" id="fileList">
            No raster selected
          </div>
        </div>

        <button class="btn w-full"id="submitFilesButton" onclick="processFiles()"><span class="material-icons">play_arrow</span> Start Georeference</button>
      </section>
      ...
      <section class="input-box" role="tabpanel" aria-labelledby="tab-about" id="about" tabindex="0" hidden>
        ...
        <ul>
          <li>
            The scanned map file name must begin with <strong>IDSLS</strong>, for example:
            <code>64710500010001.jpg</code>, <code>64710500010001_WS.jpg</code>.
            The program will take the first 14 digits of the file name to match it with the
            IDSLS in the digital SLS master.
          </li>
          <li>
            The WS map scan result must be of good quality, with no folded paper, especially in the map container area, as this is the part read by the computer vision program.
          </li>
          <li>
            The WS map scan must not be upside down.
          </li>
        </ul>
        <h4>Quick Guide</h4>
        <h5>From file</h5>
        <ol>
          ...
          <li>Select map type <strong>(WS/WB map)</strong></li>
        ...
        <p><b>nahar.nasrullah@bps.go.id</b></p>
...
function getDbConfig(){
  ...
  //   alert('Please fill in Host, Database Name and User Name to connect.');
  //   return;
  // }
...
window.refreshMasterMapsSelect = function(){
  ...
  GetMasterMaps().then((masterMaps) => {
    ...
    // const option = document.createElement('option');
    // option.textContent = "error during load data";
    // masterMapSelect.appendChild(option);
...
window.selectGeojson = function () {
    ...
    const masterMapType = document.querySelector('input[name="masterMapType"]:checked').value;
        ...
        .then((result) => {
          ...
          selectedGeojson=result;

          const listItem = document.createElement('div');
          listItem.setAttribute('title', result);
          var filename = result.replace(/^.*[\\/]/, '');
          listItem.innerHTML = '<span>'+filename+'</span>' + '<button class="x"data-path=\"'+result+'\"onclick="removeGeojson(this)">x</button>';
...
window.processFiles = function () {
    ...
    try {
        ...
        ProcessFiles(selectedFiles,selectedMap,masterMapType,masterMapSource)
        .then((result) => {
            resetForm();
            log.innerHTML ="";
            result.forEach(function(item, index) {
...
window.removeFile = function (e) {
    ...
    if(selectedFiles.length == 0){
      ...
      fileList.innerHTML = "No raster Selected"
    }
};

window.removeGeojson = function (e) {
    var fpath = e.getAttribute("data-path");
    e.parentNode.parentNode.removeChild(e.parentNode);
    selectedGeojson ="";
    geojsonFile.style.display = "none";
    geojsonFile.innerHTML = "No geojson Selected"
};
